Chunk.prototype.animate = function(options) {
...
Chunk.prototype.draw = function(progress){
  this.style.left = progress + 'px';
}

function animateChunks(chunk)
{
	for(var i in chunk)
	{
		setTimeout(
			animate({
				duration: chunk[i].settings.duration,
				timing: chunk[i].settings.easing,
				draw: chunk[i].draw(progress);
			}), 
			chunk[i].settings.delay;
		);
	}
}
		...
		for(var i in parms)
		{
			...
			switch (the_parm[0])
			{
				...
				case 'auto':
					...
					break;
				case 'scrolled':
					this.offsetTop =  $(this.elem).offset().top + $(this.elem).innerHeight();
					//$(this.elem).attr('ofsetTop', this.offsetTop);
					//$(this.elem).attr('ofset', $(this.elem).offset().top);
					//$(this.elem).attr('h', $(this.elem).innerHeight());
					Scrolled.push(this);
					break;
				case 'delay': 
...
function drawScrolled(offsetScroll)
{
	for(var i in Scrolled){
		if(offsetScroll >= Scrolled[i].offsetTop){
			Scrolled[i].draw();
		}
	}
}
...
$(document).ready(function(){
	...
	//$('body').append('<div id="consol"></div>');
	window.onscroll = function()
	{
		var offsetScroll = (window.pageYOffset || document.documentElement.scrollTop) + window.innerHeight;
		//$('#consol').html(offsetScroll);
		drawScrolled(offsetScroll);
	};
});